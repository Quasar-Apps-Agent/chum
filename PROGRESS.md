...
## ENVIRONMENT (2026-09-06, blocks the Mac lane) — OWNER ACTION
The repo lives in the iCloud-synced Desktop and the Mac has ~16 GB free, so
iCloud "Optimize Mac Storage" evicts project files to the cloud (4,544 files,
4.1 GB, found evicted; re-evicted within 25 minutes of pulling them back).
Unreal memory-maps its 126 MB asset-registry cache at startup and any evicted
.uasset on load, so every commandlet and capture stalls for 20–40 minutes.
This was the cause of every "hang" attributed to the Zen server. Fix (owner):
move the repo out of ~/Desktop (e.g. ~/Projects/restoration) or turn off
Desktop & Documents sync / Optimize Mac Storage, and give the disk headroom.
RESOLVED 2026-09-06 19:30: the repo moved to `~/Projects/restoration` (7,078 files, none evicted; imports and captures back to under a minute). The Mac lane resumed at 1.9.
...
- [x] 1.3 Collar, leather strap, dead brass bell — DONE 2026-09-05: bevelled leather band ON the pile (fabric_leather_02, CC0) with 16 rivets, buckle, prong and strap tail; hollow slotted sleigh bell (r 0.05 base → 0.129 m at 3.35, PLATE proportion) with two end holes, a black void object inside (no clapper), crown loop, stitched hanging tab, `SOCKET_Bell`; blackened crown painted as a noise-ragged tide line and BAKED (new as-is bake path) so both engines carry it; pry scratch geometry. Evidence `docs/telemetry/blender-phase1/1.3_*.png`, `docs/telemetry/ue-baselines/1.3_ue_*.png`. Pipeline: fixup sets map weights on importer-made instances (rivets and the jaw bronze band finally dark in Unreal). OPEN for owner: bell diameter (PLATE proportion used), pry-mark kept. Limits: Unreal closeup frames auto-expose hot (brass clips), the tally spill test waits for the in-game rig.
- [x] 1.4 Arms & hands: tendons both sides, articulated fingers — DONE 2026-09-06: arms rebuilt with wrist taper and inner-elbow crease; viewer-left arm torn open (boolean window) over a hessian-lined core with FOUR sagging steel-cable tendons (Rope002), THREE under cloth on the right surfacing between rusted guide blocks; bandage ribbons at the elbows; the mitt (ArmR): sewn wool palm and three one-piece curled fingers, baked; the skeletal hand (ArmL): iron plate palm, wrist bolt, four rod digits of three phalanges with knuckle bolts (Metal041B), progressive curl and splay, palms turned 35° in; horn claws (Bark015) seated in the tips; every phalanx on a knuckle pivot named to the rig spec (`finger_[a-d]_0N_[r|l]`, `hand_[r|l]`; ArmL→_r). Evidence `docs/telemetry/blender-phase1/1.4_*.png`, `docs/telemetry/ue-baselines/1.4_ue_*.png`. OPEN for owner: digit count (4 iron / 3 mitt as built), hand span (mitt ≈0.34 m, iron ≈0.25 m at 3.35 vs the brief's 0.45–0.55), which arm works the lever. Deferred: guide blocks are bevelled boxes (gate_latch_01 donor fetched, not yet cut); the lever-grip articulation proof moves to 1.11 with the pivots in place.
- [x] 1.5 Legs: control rods, torn fur windows, weighted feet — DONE 2026-09-06: leg columns with knee and ankle in the chain and a back-of-knee fold; internal rod assembly both sides (two rods, knee and ankle brackets, bolts; Metal041B) sitting in the cavity of a torn window on the viewer-right shin over a hessian core (window ≈0.46 m tall at 3.35); paws as one remeshed mass (three toe lobes, pad, sole flattened by a bite), fur cards above the rim; pale worn patch on the viewer-left foot top (BellyWool bake); weighted plinth under each paw (metal_plate_02, dark scratched plate); pivots per rig spec (`calf_/foot_/ball_[r|l]`, LegL→_r); collision: four boxes (paws, plinths) — UCX name-matching failed under combined import in every convention, so the export writes `SM_ChumAF.collision.json` and the import builds box elements (verified box=4, simple-as-complex). Evidence `docs/telemetry/blender-phase1/1.5_*.png`, `docs/telemetry/ue-baselines/1.5_ue_*.png`. OPEN for owner: plinth material (metal plate built; canon sound says wood-through-floor); which leg has the window (viewer-right built). Deferred: donor pipe cuts for rods/brackets (bevelled rods and blocks now).
- [x] 1.6 Tail: segmented core, fur, rust tip — DONE 2026-09-06: ONE continuous tapered curve mesh (point-radius taper 0.095→0.04 base, ≈1.3 m at 3.35), remeshed, its own bake (`TailWool`) with the rust tip as a soft noise-ragged colour zone via `add_tint_zone()`; fur cards and Cycles hair on the dorsal ridge and tip only; rigid mass (no secondary). Unreal rear frame: one mass, grounded, no gaps (the old joined spheres arrived as three lumps). Evidence `docs/telemetry/blender-phase1/1.6_*.png`, `docs/telemetry/ue-baselines/1.6_ue_*.png`. OPEN: exposed core hardware (none built; PLATE reads banding as fur). Pipeline note: the editor commandlet hung twice at startup on a wedged Zen derived-data server after a killed process — kill zenserver and clear `Zen/Data/.lock`; the Mac has ~16 GB free.
- [~] 1.7 Full-figure unification + texture/size budget — TRANCHE 1 DONE 2026-09-06: sockets `SOCKET_JawLever`, `SOCKET_EyeTally` (with `SOCKET_Bell`, `SOCKET_ThroatSpeaker`) exported; ORM packing at export (R=AO, G=baked roughness or constant, B=metallic; 51 materials); a real base master `M_ChumAF_Base` (BaseColor/Normal/ORM → physical inputs) with instances on 49 of 64 slots — the importer's Phong master is retired for every textured slot, metals and roughness finally physical; every scan normal now imports as a normal map (they were sRGB colour since 0.3) and masks as linear. Evidence `docs/telemetry/ue-baselines/1.7_ue_*.png`. REMAINING (needs owner rulings, OPEN): budget numbers (brief proposal FBX ≤25 MB, ≤20 maps, ≤300k tris) vs measured (see ledger Commit 115); material grouping/atlases to ≤12 instances; the `T_ChumAF_*_BC/_N/_ORM` naming pass; whether the Godot glb keeps 2.6 m; the 15 untextured slots (flat Phong colours) fold into the wool atlas.
- [x] 1.8 TRANCHE 2B DONE 2026-09-06 (UNIT 1.8 CLOSED): chin bands as bevelled plates, hinge bolts as bevelled rods with proud heads, jaw bars and the lens mount as hand-bent wobble rings (no tori, no scaled cubes); curvature edge wear (`edge_wear()`, Bevel-vs-normal) in BronzeBand/CopperRing/RodMetal, baked AS-IS per piece so the engine's maps carry it (a shared-material metallic bug fixed: the as-is bake now restores the source's metallic); fur atlas regenerated denser, shorter, clumped, singed tips (`make_fur_cards.py`), `M_FurCards` roughness breaks with the tuft alpha. Evidence `docs/telemetry/blender-phase1/1.8c_*`, `docs/telemetry/ue-baselines/1.8c_ue_head_tally{0,1}.png`. Engine full frames captured after the relocation (`docs/telemetry/ue-baselines/1.8c_ue_full_tally{0,1}.png`). Open by design: the button eye keeps its gear rosette (reads matte horn); lip staples stay geometry.
- [~] 1.8 TRANCHE 2A DONE 2026-09-06: face quilt panels rebuilt as SOLID embedded patches on the skull (`solid_patch(host=skull)`, 8 panels, baked with PanelA/B/C/PatchRust/CharDark tints — the thin shells are gone); 38 stitch/staple objects (crown seam, face seams, X-ticks, border, ear edge stitches) PROJECTED into the skull and ear maps by a selected-to-active pass after the burlap bake (`DETAIL_INTO`), then deleted — `docs/telemetry/blender-phase1/1.8b_skull_nrm_stitches.png` shows the bumps; lip staples stay geometry (silhouette). Engine: `docs/telemetry/ue-baselines/1.8b_ue_*`. PIPELINE: the editor commandlet hung a third time on the Zen derived-data server; every engine invocation now runs `-ddc=NoZenLocalFallback` (file-based cache; the first capture rebuilt global Metal shaders for 36 min, warm since). REMAINING: bevelled chin bands/jaw bars/hinge/lens ring with edge wear (step 4); fur atlas regen + anisotropy (step 6).
- [~] 1.8 TRANCHE 1 DONE 2026-09-06: tally core baked into the asset (`TallyCore`, emissive; `SOCKET_EyeTally` at its centre), `M_TallyCore` in the engine with `TallyIntensity` 0↔lit and `MI_TallyCore_Lit` — dark portrait shows a dead barrel, lit burns red through it, red reads at gameplay distance (`docs/telemetry/ue-baselines/1.8_ue_*_tally{0,1}.png`); teeth remeshed as chipped slats (cube slats gone; scan swapped to stained ivory for the next build, the crackle read as wood); jaw SHUT in every shipped frame per CAPTURE CANON, one labelled design frame for the maw; sodium `--frame head`. REMAINING: face quilt panels solid + baked; staples/stitches → maps (≈70 cylinders); bevelled chin bands/jaw bars/hinge/lens ring with edge wear; fur atlas regen + anisotropy; the tally core clipped white at strength 20 (set 6 for the next build).
...
- [~] 1.9 TRANCHE 2 DONE 2026-09-06: `tools/anim_chum_af.py` authors the five clips on the rig — `A_ChumAF_Parked` (one frame), `A_ChumAF_Pour_Walk` (52 f @30, root 0.81 m/s, stride 1.40 m PROPOSED, swing ±17°, knees ≤9°, head leads plants by 3 f, 333 LINEAR keys / 126 BEZIER confined to the 2-frame contact caps), `A_ChumAF_Pour_Cross` (time-scaled, 1.62 m/s), `A_ChumAF_Stop`/`A_ChumAF_Start` (2 f); curve audit `docs/telemetry/ue-baselines/1.9b_anim_curve_audit.txt`; measured geometric head bob 5.2 cm vs the brief's 3 cm proposal — OPEN for the owner with the stride. `export_ue.py --anim` (one FBX per action, header units), `import_fbx.py` imports `A_` files onto `SK_ChumAF_Skeleton` (delete-then-import; the Interchange path converts the armature node and gives it scale 100 over metre bone tracks — no rescaling, every variant tried broke it). The rig's root is the armature OBJECT named `root` (the engine's top bone); 35 bones under it. `stage_and_capture.py` gained `UE_CAPTURE_ANIM`/`UE_CAPTURE_SEQ` (a throwaway level sequence scrubbed per frame — an unticked editor world evaluates nothing else); eight-frame walk sheet `docs/telemetry/ue-baselines/1.9b_ue_walk_sheet.png`: the legs plant, swing and plant. REMAINING (tranche 3): root travel is not visible under the Sequencer scrub though the asset holds 139.8 cm at the cycle end — verify in PIE against the brain's speed (no-slide test); sockets on the skeletal mesh post-import; physics asset; AnimBP with the eye look-at layer; the overlay/head-height/stop-diff acceptance numbers.
- [~] 1.9 TRANCHE 1 DONE 2026-09-06: `tools/rig_chum_af.py` builds `SK_ChumAF` — 36 bones on the spec's positions (root/pelvis/spine_01-02/neck/head/jaw/eye_tally/bell/ears/tail_01-02, clavicle→hand and thigh→ball per side, one bone per finger this tranche: 3 mitt + 4 iron), bone side law: build −X = rig _r; 256 meshes bound with rigid 1.0 weights and 3 cm blend bands at hip/chest/neck, knee/ankle, elbow/wrist; no physics, no cloth. `export_ue.py --skeletal`, `fixup_materials.py` and `stage_and_capture.py` are skeletal-aware; `SK_ChumAF` imports (305 MB FBX), 70 of 82 slots on the physical master, tally lit; frames `docs/telemetry/ue-baselines/1.9a_ue_sk_*.png`, pose test `docs/telemetry/blender-phase1/1.9a_rig_pose_test.png`. REMAINING (tranche 2): sockets on the skeletal mesh (the FBX importer creates none from SOCKET_ nodes on skeletal meshes — add them post-import from a sockets JSON), a physics asset for the paws/plinths (UCX boxes dropped from the skeletal build), the walk/cross/parked/stop/start clips with the curve audit, `--anim` export, the AnimBP eye layer, the frame-sequence acceptance capture.
