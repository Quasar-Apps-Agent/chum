...
- [x] 0.8b-spec [CLOUD-OK] Transcribe the FULL v16 _save_dict schema
      (all fields, types, defaults) + game_state.gd public field/signal
      inventory into ue/PORT-NOTES-STATE.md — the 0.8b implementation
      checklist, pure text work from the Godot source. DONE (cloud,
      2026-09-03): 55 keys / 74 vars / 20 signals / 47 public funcs
      transcribed with record shapes, value domains, toast text and the
      carry-across-new-game set; §7 gap audit of the 0.8a SaveGame finds 32
      missing keys, 4 wrong types, 2 wrong defaults, Strike cap 6 vs 7;
      tripwire tools/check_port_notes_state.py asserts notes ↔ source ↔ header
