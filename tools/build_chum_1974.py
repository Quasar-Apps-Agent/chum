"""CHUM · 1974 PEAK · mesh build (design doc Part I: the reference plate set)

Run:  /Applications/Blender.app/Contents/MacOS/Blender --background --python tools/build_chum_1974.py

The professional rebuild, the icon: brown boiled-wool body, triangle ears
with contrasting inner patches (mustard viewer-left, navy viewer-right),
round head with a visible center seam, one amber glass cat eye (viewer
left), one black button (viewer right), cross-stitched grin of X ticks,
felted patches in blanket stitch — rust chest, green right side, the big
tan belly circle, blue arm, plum thigh, mustard toe caps, navy tail tip —
and the thin brown collar with the brass keyhole bell that never rings.
Pristine surfaces: this one was loved and kept clean. ~1.05 m tall, faces
Godot +Z (Blender -Y). Exports assets/models/chum_1974.glb.
"""
import bpy
import math
import os

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "assets", "models", "chum_1974.glb")
...
def organic(obj, material, voxel=0.014, lump_str=0.006, fiber_str=0.003, decimate=0.5):
    bpy.ops.object.select_all(action="DESELECT")
    obj.select_set(True)
...
seam = torus((0, 0, 0.86), 0.243, 0.004, rot=(0, math.radians(90), 0))
seam.name = "HeadSeam"
simple(seam, M["Thread"])

for sx, inner in ((-1, "Mustard"), (1, "Navy")):
    e = cone((0.145 * sx, 0.01, 1.1), 0.105, 0.2, scale=(1, 0.38, 1),
             rot=(0, math.radians(10 * sx), 0))
    e.name = "Ear" + ("L" if sx < 0 else "R")
    organic(e, M["WoolBrown"], 0.008, 0.004, 0.002, 0.55)
    ip = cone((0.14 * sx, -0.035, 1.08), 0.055, 0.1, scale=(1, 0.3, 1),
              rot=(0, math.radians(10 * sx), 0))
    ip.name = "EarInner" + ("L" if sx < 0 else "R")
    simple(ip, M[inner])
...
for i in range(7):
    t = (i - 3) / 3.0
    px = 0.11 * t
    pz = 0.775 + 0.028 * (t * t)   # gentle upward smile
    py = -0.235 + 0.02 * abs(t)
    for ang in (45, -45):
        tick = cyl((px, py, pz), 0.0028, 0.026,
                   rot=(0, math.radians(ang), 0))
        tick.name = f"GrinX{i}_{ang}"
        simple(tick, M["Thread"])

# twisted-string whiskers, three per side, drooping softly
for sx in (-1, 1):
    for k in range(3):
        w = cyl((0.19 * sx, -0.17, 0.83 + 0.028 * k), 0.0035, 0.2,
                rot=(math.radians(4 * (k - 1)), math.radians(-72 * sx), 0))
        w.name = f"Whisker{'L' if sx < 0 else 'R'}{k}"
...
# ---- collar and the bell that never rings ---------------------------------------------------
collar = torus((0, 0, 0.665), 0.155, 0.018, rot=(math.radians(4), 0, 0))
collar.name = "Collar"
simple(collar, M["CollarBr"])
bell = sphere((0, -0.16, 0.62), 0.035)
bell.name = "Bell"
simple(bell, M["Brass"])
